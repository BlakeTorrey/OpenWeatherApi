...
const tempUnitConversion = (temperature: number): number => {
  return Math.round((temperature - 273.15) * (9 / 5) + 32);
}

function dateConversion(jsonDate: number): Date {
  let date = new Date(jsonDate * 1000);
  return date;
}
...
const renderCurrentWeather = (currentWeather: any): void => {
  ...
  const tempConversion = tempUnitConversion(tempF);
  const convertedDate = dateConversion(date);
  const modifiedDate = convertedDate.toTimeString();

  // convert the following to typescript
  heading.textContent = `${cityName} (${modifiedDate})`;
  ...
  tempEl.textContent = `Temp: ${tempConversion}°F`;
...
const renderForecastCard = (forecast: any) => {
  const { cityName, date, icon, iconDescription, tempF, windSpeed, humidity } = forecast;
  ...
  const { col, cardTitle, weatherIcon, tempEl, windEl, humidityEl } =
    ...
    const tempConversion = tempUnitConversion(tempF);
  const convertedDate = dateConversion(date);
  const modifiedDate = convertedDate.toDateString();
  // Add content to elements
  cardTitle.textContent = `${cityName}, ${modifiedDate}`;
  weatherIcon.setAttribute(
    'src',
    `https://openweathermap.org/img/w/${icon}.png`
  );
